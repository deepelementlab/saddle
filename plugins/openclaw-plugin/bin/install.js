...
/**
 * Saddle OpenClaw Plugin — configures ~/.openclaw/openclaw.json
 */
...
const PLUGIN_ID = "saddle-ai-openclaw";
const PLUGIN_DIR = path.join(__dirname, "..");
const STABLE_PLUGIN_DIR = path.join(HOME_DIR, ".openclaw", "plugins", "saddle-ai-openclaw");
const DEFAULT_CONFIG = {
  baseUrl: "http://127.0.0.1:1995",
  userId: "saddle-user",
  groupId: "saddle-default-group",
  topK: 5,
...
function isDuplicatePluginPath(entry, currentLoadPath) {
  ...
  return (
    normalized.includes("@saddle/openclaw-plugin") ||
    normalized.includes("/plugins/openclaw-plugin") ||
    normalized.includes("/saddle-ai-openclaw")
  );
}
...
function mergePluginConfig(existingConfig, overrides = {}) {
  ...
  if (Array.isArray(cleaned.memoryTypes)) {
    cleaned.memoryTypes = cleaned.memoryTypes.filter((t) => t === "episodic_memory");
...
async function install() {
  hr();
  log("Saddle OpenClaw Plugin Installer", "blue");
  log("Requires: saddle serve (python -m saddle serve) on the configured baseUrl.");
  hr();
  ...
  if (hadExistingConfig) {
    ...
    hr();
    info("Enter settings for Saddle backend (saddle serve).");
    const baseUrl = await promptWithDefault("Saddle base URL", DEFAULT_CONFIG.baseUrl);
    const health = await checkBackendHealth(baseUrl);

    if (health.ok) {
      success(`Saddle backend is reachable (${health.status || "ok"}).`);
    } else {
      warn(`Saddle backend is not reachable yet (${health.reason || "unknown"}).`);
      warn("You can continue; start later with: python -m saddle serve");
    }
